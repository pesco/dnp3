...
uint16_t compute_crc(HCountedArray *bytes)
{
    ...
    for(size_t i=0; i<bytes->used; i++) {
        uint8_t databyte = H_CAST_UINT(bytes->elements[i]);
        crc = (crc>>8) ^ table[((uint8_t)crc ^ databyte) & 0xFF];
    }
    crc = ~crc; // invert
